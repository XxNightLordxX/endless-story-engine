...
/**
 * Dynamic keywords interface for web-search based vocabulary
 */
interface DynamicKeywords {
  time: string[];
  location: string[];
  environment: string[];
  population: string[];
...
export class DynamicWorldSimulation {
  ...
  private dynamicKeywords: DynamicKeywords | null = null;
  private keywordsSearchCache: Set<string> = new Set();
  ...
  async updateFromChapter(
    ...
    // Extract world information from chapter (now async with web search)
    const worldInfo = await this.extractWorldInformation(chapter.content);
  ...
  /**
   * Extract world information from chapter content using dynamic keywords
   */
  private async extractWorldInformation(content: string): Promise<{
    time: string[];
    location: string[];
    environment: string[];
    population: string[];
    ...
    // Ensure we have dynamic keywords loaded
    if (!this.dynamicKeywords) {
      await this.generateDynamicKeywords();
    }
    
    const keywords = this.dynamicKeywords || this.getFallbackKeywords();
    
    return {
      time: this.extractKeywords(content, keywords.time),
      location: this.extractKeywords(content, keywords.location),
      environment: this.extractKeywords(content, keywords.environment),
      population: this.extractKeywords(content, keywords.population),
      events: this.extractKeywords(content, keywords.events),
      weather: this.extractKeywords(content, keywords.weather),
      magic: this.extractKeywords(content, keywords.magic)
    };
  }

  /**
   * Generate dynamic keywords via web search
   */
  private async generateDynamicKeywords(): Promise<void> {
    console.log('  Generating dynamic world keywords via web search...');
    
    try {
      const searches = await Promise.all([
        this.searchKeywords('fantasy story time periods day night dawn dusk morning evening', 5),
        this.searchKeywords('fantasy locations places forest castle town city mountain river', 5),
        this.searchKeywords('fantasy environment weather cold hot warm dry humid windy dark bright', 5),
        this.searchKeywords('fantasy characters population people army faction king queen soldier merchant', 5),
        this.searchKeywords('fantasy events war battle festival ceremony celebration crisis', 5),
        this.searchKeywords('fantasy weather rain snow wind storm fog mist sunny cloudy', 5),
        this.searchKeywords('fantasy magic spell enchantment curse ritual summon conjure supernatural', 5),
      ]);
      
      this.dynamicKeywords = {
        time: this.parseKeywordsFromSearch(searches[0], ['morning', 'afternoon', 'evening', 'night', 'dawn', 'dusk']),
        location: this.parseKeywordsFromSearch(searches[1], ['forest', 'castle', 'town', 'city', 'mountain', 'river']),
        environment: this.parseKeywordsFromSearch(searches[2], ['cold', 'hot', 'warm', 'dry', 'humid', 'windy', 'dark', 'bright']),
        population: this.parseKeywordsFromSearch(searches[3], ['people', 'army', 'faction', 'king', 'queen', 'soldier', 'merchant']),
        events: this.parseKeywordsFromSearch(searches[4], ['war', 'battle', 'festival', 'ceremony', 'celebration', 'crisis']),
        weather: this.parseKeywordsFromSearch(searches[5], ['rain', 'snow', 'wind', 'storm', 'fog', 'mist', 'sunny', 'cloudy']),
        magic: this.parseKeywordsFromSearch(searches[6], ['magic', 'spell', 'enchantment', 'curse', 'ritual', 'summon', 'conjure']),
      };
      
      console.log('  Dynamic keywords generated successfully');
    } catch (error) {
      console.error('Error generating dynamic keywords:', error);
      this.dynamicKeywords = this.getFallbackKeywords();
    }
  }

  /**
   * Search for keywords using web search
   */
  private async searchKeywords(query: string, numResults: number): Promise<any> {
    const cacheKey = query.toLowerCase().substring(0, 40);
    if (this.keywordsSearchCache.has(cacheKey)) {
      return { results: [] };
    }
    this.keywordsSearchCache.add(cacheKey);
    
    try {
      return await webSearch(query, numResults);
    } catch (error) {
      console.error(`Keyword search failed for "${query}":`, error);
      return { results: [] };
    }
  }

  /**
   * Parse keywords from search results
   */
  private parseKeywordsFromSearch(searchResults: any, fallback: string[]): string[] {
    const keywords: string[] = [...fallback];
  ...
  /**
   * Get fallback keywords if web search fails
   */
  private getFallbackKeywords(): DynamicKeywords {
    return {
      time: ['morning', 'afternoon', 'evening', 'night', 'dawn', 'dusk', 'day', 'hours', 'minutes'],
      location: ['forest', 'castle', 'town', 'city', 'mountain', 'river', 'sea', 'village', 'palace', 'cave'],
      environment: ['cold', 'hot', 'warm', 'dry', 'humid', 'windy', 'still', 'dark', 'bright', 'sunny', 'cloudy'],
      population: ['people', 'citizens', 'army', 'faction', 'rebel', 'king', 'queen', 'soldier', 'merchant', 'farmer'],
      events: ['war', 'battle', 'festival', 'ceremony', 'celebration', 'riot', 'protest', 'revolution', 'crisis'],
      weather: ['rain', 'snow', 'wind', 'storm', 'clear', 'cloudy', 'fog', 'mist', 'sunny'],
      magic: ['magic', 'spell', 'enchantment', 'curse', 'ritual', 'summon', 'conjure', 'magical']
    };
  }
